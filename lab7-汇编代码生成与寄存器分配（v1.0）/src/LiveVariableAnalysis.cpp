...
{
    /*
    set_difference,所有在第一个集合中但不在第二个集合中的，
    在这里把所有使用的操作数分成了def和use两类
    */
...
{
    /*
    对一个函数中每一个汇编代码基本块和汇编指令，
    获取了这个指令所使用的操作数，并建立起了all_uses的映射关系
    */
    for (auto &block : func->getBlocks())
    {
        for (auto &inst : block->getInsts())
        {
            ...
            for (auto &use : uses)
                /*
                all_uses[*use]是use这一个操作数再alluses map容器中所对应的set集合
                */
