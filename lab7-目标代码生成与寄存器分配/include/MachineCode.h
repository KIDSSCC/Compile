...
private:
    ...
    int type;
    int val;  // value of immediate number 拓float不？
...
public:
    ...
    */
    enum instType { BINARY, LOAD, STORE, MOV, BRANCH, CMP, STACK };//挪这里
    enum condType { EQ, NE, LT, GE , GT, LE, NONE };//这个地方WA debug
...
public:
    ...
    二元运算指令，加减乘除与或
    */
    enum opType { ADD, SUB, MUL, DIV, MOD,AND, OR };//arm貌似没有除法和取模？六种关系运算也不用？
...
public:
    ...
    movm指令，暂时存疑
    */
    enum opType { MOV, MVN };//MVN:传送之前，将被传送的对象先按位取反，再传送到目的寄存器
...
public:
    ...
    */
    ...
    StackMInstrcuton(MachineBlock* p, int op, std::vector<MachineOperand*> srcs,MachineOperand* dst,MachineOperand* src = nullptr,int cond = MachineInstruction::NONE);
    void output();
};
